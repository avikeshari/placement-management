...
const Jobs = () => {
  ...
  const doArchive = async () => {
    ...
    try { await api.delete(`/jobs/${archiveTarget._id}`); setJobs((prev) => prev.filter((item) => item._id !== archiveTarget._id)); toast.success("Job archived successfully"); }
    catch (error) { toast.error(getErrorMessage(error, "Unable to archive job.")); }
  ...
  return (
    <section>
      ...
      <ConfirmDialog
        ...
        message={archiveTarget ? `Archive "${archiveTarget.title}"? The job will be closed and hidden from students. Applications and interview history will be preserved.` : ""}
