...
export default function Drives() {
  ...
  const create = async (e) => {
    ...
    try { await api.post("/drives", form); toast.success("Placement drive created"); setForm({ name: "", description: "", startAt: "", endAt: "", location: "", companies: [] }); load(); }
  ...
  return <section>
    ...
    <form onSubmit={create} className="bg-white border rounded-2xl p-6 mt-6 grid md:grid-cols-2 gap-4">
      <input required placeholder="Drive name" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} className="border rounded-lg px-3 py-2.5" />
      <input placeholder="Location / venue" value={form.location} onChange={e => setForm({ ...form, location: e.target.value })} className="border rounded-lg px-3 py-2.5" />
      ...
      <textarea placeholder="Description" value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} className="md:col-span-2 border rounded-lg px-3 py-2.5" />
      ...
      <button className="md:col-span-2 bg-blue-600 text-white rounded-lg px-5 py-2.5">Create Placement Drive</button>
    </form>
    ...
    </article>) : <p className="text-slate-500">No placement drives created yet.</p>}</div>
