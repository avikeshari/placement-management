...
const Companies = () => {
  ...
  return <section>
    <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-6"><div><h1 className="text-2xl md:text-3xl font-bold">Companies</h1><p className="text-slate-500 mt-2">Monitor company accounts and hiring activity.</p><button onClick={async()=>{try{const r=await api.get("/admin/companies/export",{responseType:"blob"});const u=URL.createObjectURL(r.data);const a=document.createElement("a");a.href=u;a.download="company-database.csv";a.click();URL.revokeObjectURL(u)}catch(e){toast.error(getErrorMessage(e,"Unable to export company database."))}}} className="mt-3 bg-blue-600 text-white px-4 py-2 rounded-lg">Export Company Database</button></div><div className="relative w-full md:w-80"><Search size={18} className="absolute left-3 top-3 text-slate-400" /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search companies" className="w-full border rounded-lg pl-10 pr-3 py-2.5 bg-white" /></div></div>
